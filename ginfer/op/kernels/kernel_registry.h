...
class KernelRegistry {
 public:
  inline static std::unordered_map<DeviceType, std::shared_ptr<KernelRegistry>> registries_ =
    {{DeviceType::kDeviceCPU, nullptr}, 
     {DeviceType::kDeviceCUDA, nullptr}};
...
#define INSTANTIATE_KERNEL_FUNC(tpl_func, dev_type, dtype) \
  &tpl_func<typename common::DeviceDType<dev_type, dtype>::type, typename common::DeviceContextType<dev_type>::type>

#define _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype) \
  _REGISTER_KERNEL_SAME_DTYPE(name, dev_type, INSTANTIATE_KERNEL_FUNC(tpl_func, dev_type, dtype), dtype)

#define _REG_TPL_KRNL_DT_1(name, dev_type, tpl_func, dtype) _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype)
#define _REG_TPL_KRNL_DT_2(name, dev_type, tpl_func, dtype, ...) _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype) _REG_TPL_KRNL_DT_1(name, dev_type, tpl_func, __VA_ARGS__)
#define _REG_TPL_KRNL_DT_3(name, dev_type, tpl_func, dtype, ...) _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype) _REG_TPL_KRNL_DT_2(name, dev_type, tpl_func, __VA_ARGS__)
#define _REG_TPL_KRNL_DT_4(name, dev_type, tpl_func, dtype, ...) _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype) _REG_TPL_KRNL_DT_3(name, dev_type, tpl_func, __VA_ARGS__)
#define _REG_TPL_KRNL_DT_5(name, dev_type, tpl_func, dtype, ...) _REGISTER_TPL_KERNEL(name, dev_type, tpl_func, dtype) _REG_TPL_KRNL_DT_4(name, dev_type, tpl_func, __VA_ARGS__)
...
#define REGISTER_KERNEL(name, dev_type, tpl_func, ...)                                     \
  _GET_REGISTER_KERNEL_MACRO(__VA_ARGS__, _REG_TPL_KRNL_DT_5, _REG_TPL_KRNL_DT_4,          \
      ...
      name, dev_type, tpl_func, __VA_ARGS__);
