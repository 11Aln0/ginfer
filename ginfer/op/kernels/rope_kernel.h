...
template <typename T, typename Context>
void Llama3RotaryEmbeddingKernel(const Context& ctx, tensor::Tensor& sin_cache, tensor::Tensor& cos_cache, int start_pos,
                                 int end_pos, float rope_theta, float factor, float high_freq_factor,
...
using Llama3RotaryEmbeddingKernelFuncType = decltype(&Llama3RotaryEmbeddingKernel<float, common::DeviceContext>);
