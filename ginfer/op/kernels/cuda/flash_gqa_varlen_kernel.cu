...
__device__ __forceinline__ void loadKVPagedTile(const T* __restrict__ p_global,
  ...
  for(int r = 0; r < bc; r += seqs_per_iter) {
    ...
    int btbl_idx = logical_seq / paged_block_size; // TODO bit operator if paged_block_size is power of 2
...
__device__ __forceinline__ void computePTile(T* p_SP, /** S/P ptr of smem */
  ...
  for(int r = 0; r < bound; r += row_per_iter) {
    ...
    for(int j = 0; j < vec_size; j++) {
      ...
      bool is_valid = global_c <= (global_r + casual_mask_offset) && global_c < kv_seqlen; // casual mask and seqlen mask
...
__global__ void GQAVarlenKernelImpl(const T* __restrict__ q,
  ...
  // load Q FA Tile
  loadQTile<T, br, head_dim>(q + q_gmem_offset, p_q_block_smem, min(br, q_seqlen - q_tile_id * br), q_seqlen_stride);
  __syncthreads();
  ...
  for(int c = 0; c < kv_seqlen; c += bc) {
    ...
    computePTile<T, br, bc, head_dim>(p_s_block_smem, p_old_m, p_new_m, p_l_smem, q_seqlen, kv_seqlen, global_r_base, global_c_base);
    __syncthreads();
  ...
  storeOTile<T, head_dim>(p_o_block_smem, output + out_gmem_offset, p_l_smem, min(br, q_seqlen - q_tile_id * br), out_seqlen_stride);
  
  // __syncthreads();
  // if(threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0) {
  //   for(int i = 0; i < 16; i++) {
  //     printf("%d: q: %f, k: %f, v: %f, o: %f\n", i, static_cast<float>(q[i]), static_cast<float>(k[i]), static_cast<float>(v[i]), static_cast<float>(output[i]));
  //   }
  // }
  // store O block
}
  ...
  auto dispatcher = [&] () {                 
    ...
    switch(head_dim) {
      case 64:
        ...
        GQAVarlenKernelImpl<T, br, bc, 64><<<grid_dim, block_dim, smem_size, cuda_ctx.getStream()>>>(
                p_q, p_k, p_v, p_output, 
                p_cu_seqlens_q, p_cu_seqlens_kv, p_block_tables,
                num_heads, kv_heads,
      ...
      case 128:
        err = cudaFuncSetAttribute(GQAVarlenKernelImpl<T, br, bc, 128>, 
                                               ...
                                               smem_size);
        CHECK(err == cudaSuccess) << "Failed to set cudaFuncAttributeMaxDynamicSharedMemorySize";
        GQAVarlenKernelImpl<T, br, bc, 128><<<grid_dim, block_dim, smem_size, cuda_ctx.getStream()>>>(
                p_q, p_k, p_v, p_output, 
                p_cu_seqlens_q, p_cu_seqlens_kv, p_block_tables,
                num_heads, kv_heads,
                block_table_len, paged_block_size);
