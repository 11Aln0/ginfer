...
__forceinline__ __device__ void block_reduce_argmax(float& val, int64_t& idx, 
...
void argmaxKernel(const Context& ctx,
  ...
  argmaxKernelImpl<T, vec_size><<<grid_dim, block_dim, 0, cuda_ctx.getStream()>>>(
    input_data,
    output_idx_data,
    size
  );
