...
void CUDADeviceAllocator::free(void* ptr, size_t size) {
  ...
  CHECK_EQ(err, cudaSuccess) << "cudaFree failed: " << cudaGetErrorString(err);  // should not happen
}

void CUDADeviceAllocator::memcpy(const void* src, void* dst, size_t size, MemcpyKind kind, void* stream,
                                 bool sync) const {
