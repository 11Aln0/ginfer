...
Result<void, std::string> AttentionLayer::forward(const common::InferContext& ctx,
  ...
  gqa_out =
      gqa_out->reshape(hidden_state->shape());  // reshape back to [seq_len, num_heads * head_dim]
