...
void rmsNormKernel(const Context& ctx,
  ...
  if (cuda_ctx.getStream() == nullptr) {
    rmsNormKernelImpl<T>
        <<<grid_dim, block_dim>>>(in_data, gamma_data, out_data, hidden_dim, epsilon);
  } else {
    rmsNormKernelImpl<T><<<grid_dim, block_dim, 0, cuda_ctx.getStream()>>>(
        in_data, gamma_data, out_data, hidden_dim, epsilon);
  }
}
