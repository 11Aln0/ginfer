...
Result<TensorRef, std::string> Tensor::create(DataType dtype,
                                              ...
                                              DeviceType dev_type,
                                              uint8_t alloc_flags) {
  return create(dtype, std::move(shape), memory::getDeviceAllocator(dev_type, alloc_flags));
}
...
DeviceType Tensor::devType() const { return buffer_->devType(); }
